...
% this is the general file that GASA_graphtest and dot_graph_color extend
% from ---> i would like to move everything into this file, must okay with
% teresa
...
for ifile=1:size(datafiles,1)
    ...
    if  strcmp(data.ghost.ghostx, 0) == 1
        ...
        fluxx= (cts(ifile,1)./data.astrom.exptime);
        ghostmag(ifile,1)=(-2.5*log10(fluxx)+20);
...
for ifile=1:size(ndatafiles,1)
    ...
    if strcmp(data.ghost.ghostx , '') == 1 || data.ghost.ghostx == 0
        ...
        fluxx= (cts((ifile+16),1)./data.astrom.exptime);
        ghostmag((ifile+16),1)=(-2.5*log10(fluxx)+20);
