...
def create_map(ref_head, nu):
    ...
    filenames = ['IRIS_combined_SFD_really_nohole_nosource_4_2048.fits']
...
def read_in_fits(filename, ref_head):
    ...
    fits.setval(filename, 'COORDSYS', value='GALACTIC', ext=1)
    ...
    # # Plot data from healpix map
    # import healpy as hp
    # # Return projected map as numpy array
    # mask = hp.mollview(data,return_projected_map=True,xsize=10000)
    # # Plot map with negative points marked
    # plt.imshow(mask)
    # plt.scatter(np.where((mask < 0) & (mask > -np.inf))[1],np.where((mask < 0) & (mask > -np.inf))[0],s=50,c='xkcd:orange')
    # # Plot map in log with negative points shown as white
    # plt.imshow(np.log10(mask))
    # cbar = plt.colorbar()
    # cbar.set_label(r'log(IRIS/SFD 100 $\mu$m) [log(MJy/sr)]')
